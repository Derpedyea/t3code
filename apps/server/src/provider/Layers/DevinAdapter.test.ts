...
it.layer(devinAdapterTestLayer)("DevinAdapterLive", (it) => {
  ...
  it.effect("handles multi-question Devin private elicitation answers", () =>
    Effect.gen(function* () {
      const threadId = ThreadId.make("devin-private-elicitation-multi");
      const wrapperPath = yield* Effect.promise(() =>
        makeMockDevinWrapper({
          T3_ACP_EMIT_DEVIN_PRIVATE_ELICITATION_MULTI: "1",
        }),
      );
      const adapter = yield* makeTestAdapter(wrapperPath);
      const requested =
      ...
      const sendTurnFiber = yield* adapter
        .sendTurn({ threadId, input: "ask several Devin questions", attachments: [] })
        .pipe(Effect.forkChild);
      ...
      assert.deepEqual(
        ...
        [
          {
            id: "q0",
            question: "What scope of the server should become Rust?",
            options: ["Full server rewrite", "Core runtime only"],
            required: true,
            multiSelect: false,
          },
          {
            id: "q1",
            question: "Which migration strategy should the plan assume?",
            options: ["Greenfield rewrite", "Module-by-module"],
            required: true,
            multiSelect: false,
          },
          {
            id: "q2",
            question: "How should TypeScript contracts be handled?",
            options: ["Generate Rust from schemas", "Keep as a JSON boundary"],
            required: true,
            multiSelect: false,
          },
          {
            id: "q3",
            question: "What is the Rust goal?",
            options: ["Single native binary", "Reliability and safety"],
            required: true,
            multiSelect: false,
          },
        ],
      ...
      yield* adapter.respondToUserInput(
        ...
        ApprovalRequestId.make(String(requestedEvent.requestId)),
        {
          q0: "Full server rewrite",
          q1: "Greenfield rewrite",
          q2: "Generate Rust from schemas",
          q3: "Single native binary",
        },
      );
