...
export function devinModels(catalog: Catalog): ServerProviderModel[] {
  return selectableFamilies(catalog).flatMap((family): ServerProviderModel[] => {
    ...
    if (family.aliases !== undefined) model = { ...model, aliases: family.aliases };
    if (family.fusion !== undefined) model = { ...model, fusion: family.fusion };
    if (variants.some((variant) => variant.is_new)) model = { ...model, badge: "new" };
    return [model];
  });
}
